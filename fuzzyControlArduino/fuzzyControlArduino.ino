...
void loop() {
  ...
  if ((current_time - previous_time) > DELTA_T) {
    ...
    analogWrite(LEFT_EN, min(abs((int)(pwm_command * 2.25)), 255));
...
void setupFuzzyLogic() {
  ...
  FuzzySet* theta_bal = new FuzzySet(
    theta_input_scale * (-10 + theta_input_bias),
  ...
  FuzzySet* dtheta_neg = new FuzzySet(
    dtheta_input_scale * (-45 + dtheta_input_bias),
    dtheta_input_scale * (-15 + dtheta_input_bias),
    dtheta_input_scale * (-15 + dtheta_input_bias),
